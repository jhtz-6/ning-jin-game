...
/**
 * @Author: myf
 * @CreateTime: 2023-12-16  15:48
 * @Description: RequestLogUtil
 */
@Slf4j
public final class RequestLogUtils {

    private static final Logger LOGGER = LoggerFactory.getLogger(RequestLogUtils.class);
    ...
    public static RequestLogDTO buildRequestLogByHttpRequest(HttpServletRequest request) {
        ...
        try {
            ...
            requestLogDTO.setUserName("匿名用户");
        } catch (Exception e) {
    ...
    private static String getIpByRequest(HttpServletRequest request) {
        ...
        if(StringUtils.isBlank(ip)){
            log.error("RequestLogUtils.getIpByRequest.error:{}",request);
            return "127.0.0.1";
        }
        if (ip.indexOf(",") > 0) {
            ip = ip.split(",")[0];
